...
odseq <- c(0, 0.001, 0.005, 0.01)
nseq <- round(seq(20, 200, length = 100))
...
for (index in 1:nrow(dat)) {
  err_me[index] <- oracle_miss(n = dat$n[index], ploidy = ploidy, 
                                 seq = seq, bias = dat$h[index], 
                                 od = dat$od[index], dist = dist)
  err_wasp[index] <- oracle_miss(n = dat$n_new[index], ploidy = ploidy, 
...
Oracly, it's almost always better to directly model the bias.
```{r}
